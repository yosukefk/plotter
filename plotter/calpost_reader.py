...
def calpost_cat(lst):
    # catenates list of calpost data, assuming they are continuous time
    # series


    # all the  test  making sure that datasets are compaible!

    # dict_keys(['name', 'units', 'ts', 'grid', 'y', 'x', 'v', 'ptid'])
    ...
    if any(_ < 0 for _ in overlaps ):
        ...
        msg = '; '.join([msg1, msg2])
        raise cprValueError('incompative time series: {}'.format(msg))
