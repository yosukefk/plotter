...
import warnings
class PlotterVector:
    def __init__(self, arrayu, arrayv, tstamps, 
                 projection=None, extent=None, x=None, y=None, plotter_options=None):
        """
        Manages mpl.Axes with a quiver plot

        :rtype: PlotterVector
        :param np.ndarray arrayu: 3-d array of u-component values, dimensions(t, y, x)
        :param np.ndarray arrayv: 3-d array of v-component values, dimensions(t, y, x)
        ...
        self.arru = arrayu
        self.arrv = arrayv
        self.arr = np.sqrt(arrayu*arrayu + arrayv*arrayv) # lets have a scalar just to help
        ...
        # there would be quiver_optiions!
        self.quiver_options = plotter_options.get('quiver_options', None)
        self.streamplot_options = plotter_options.get('streamplot_options', None)
    ...
    def update(self, tidx=None, footnote=None, title=None):
        ...
        arru = self.arru[tuple(idx)]
        arrv = self.arrv[tuple(idx)]
        ...
        if self.quiver_options is None:
            kwds = self.streamplot_options
        else:
            kwds = self.quiver_options

        if self.hasdata:
            # TODO update quiver
            # does this work
            if self.quiver_options is None:
                self.vec.lines.remove()
                #self.vec.arrows.remove()
                self.ax.patches = []
                self.vec = self.ax.streamplot(self.x, self.y, arru, arrv, color=arr, transform=self.projection, **kwds)
            else:
                self.vec.remove()
                self.vec = self.ax.quiver(self.x, self.y, arru, arrv, arr, transform=self.projection, **kwds)
            ...
            if self.quiver_options is None:
                self.vec = self.ax.streamplot(self.x, self.y, arru, arrv,  color=arr, transform=self.projection, **kwds)
                self.mappable = mpl.cm.ScalarMappable(norm=kwds['norm'])
            else:
                self.vec = self.ax.quiver(self.x, self.y, arru, arrv, arr, transform=self.projection, **kwds)
                self.mappable = self.vec
